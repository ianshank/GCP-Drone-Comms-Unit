...
NOTE: arming additionally requires a fresh health report. Until a live link-health
provider is wired (a later phase), ``arm`` fails closed by design.
"""
...
def build_service(
...
) -> tuple[CommandService, JsonlAuditLog]:
    """Wire the live link + audit + gate + sender + service from config/env."""
    ...
    link = MavlinkCommandLink(
        connection_factory=lambda: mavutil.mavlink_connection(endpoint),
    ...
    sender = CommandSender(
        link, audit, settings=settings, clock=clock, expect_system=target_system
    )
    service = CommandService(
        ...
        audit=audit,
        clock=clock,
        health_provider=None,  # wired in a later phase; arm fails closed until then
    )
    ...
    async def stage(request: Any) -> Any:
        ...
        if not isinstance(name, str) or not isinstance(params, dict):
            return web.json_response(
                {"error": "missing 'name' or bad 'params'"}, status=400
            )
        try:
            staged = await _run(service.stage, name, params)
        except CommandError as exc:
            return web.json_response(
                {"error": str(exc)}, status=_STATUS_FOR.get(type(exc), 400)
            )
        return web.json_response(
            {
...
def main() -> None:  # pragma: no cover - process entry point
    parser = argparse.ArgumentParser(
        description="Supervised MAVLink commanding service"
    )
    parser.add_argument(
        "--config", required=True, help="path to the commander JSON config"
    )
    ...
    service, audit = build_service(cfg, dict(os.environ))
    try:
