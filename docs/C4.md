...
flowchart TB
    subgraph node["meshsa Node (build_node from config)"]
        ...
        infer["InferenceService<br/>(NVIDIA Nemotron NIM,<br/>thread-safe session,<br/>configurable backoff/prefix,<br/>opt-in meshsa[inference])"]
        router --> infer
    end
```
...
(configurable prefix via `MESHSA_INFERENCE_INSIGHT_PREFIX`) back to the mesh. The inference
client uses thread-safe session management (`asyncio.Lock`), configurable retry backoff
(`backoff_base`), and injectable `sleep` for testability. Lazy `aiohttp` import — install
with `meshsa[inference]`; all config via `MESHSA_INFERENCE_*` env vars. The **Router** wraps
