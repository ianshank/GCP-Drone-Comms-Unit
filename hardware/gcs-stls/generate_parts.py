...
# ----------------------------------------------------------------------------
def box(length, width, height, x=0, y=0, z=0):
    """Axis-aligned box, min corner at (x,y,z)."""
    m = _box(extents=(length, width, height))
    m.apply_translation((length / 2 + x, width / 2 + y, height / 2 + z))
    return m


def boxc(length, width, height, cx, cy, cz):
...
def save(mesh, name):
    ...
    print(
        f"  {name:38s}  {dims[0]:6.1f} x {dims[1]:6.1f} x {dims[2]:6.1f} mm  "
...
tray = diff(
    tray, box(pocket_l - 2 * ledge, pocket_w - 2 * ledge, base + 2, wall + ledge, wall + ledge, -1)
)
...
xc = OLb / 2
full = add_dowel_joint(
    full,
    xc,
    [(40, OHb / 2), (OWb / 2, OHb / 2), (OWb - 40, OHb / 2)],
    boss=(28, 16, 12),
    pin_len=80,
)
...
half = box(gl, gw, gh)
half = diff(
    half, cyl(6.0, gl + 2, gl / 2, gw / 2, gh, axis="x")
)  # semicircular hinge groove on top
...
clip = diff(
    clip, cyl(5.0, depth + 2, depth / 2, (ci + 2 * w) / 2, w + ch / 2, axis="x")
)  # antenna notch end...
