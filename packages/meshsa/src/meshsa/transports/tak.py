"""TAK TCP transport: stream CoT to/from a TAK server (FreeTAKServer) over TCP
with automatic reconnect/backoff, optionally TLS-encrypted.

Pair with the ``cot`` codec (per-transport) so a node bridges a JSON mesh to a
CoT/TAK link. All network I/O is behind an injected ``connector`` seam and the
backoff ``sleep`` is injectable, so the framing/reconnect/bridge logic is tested
with fakes; only the real socket builder is ``# pragma: no cover``. Nothing is
hard-coded — host/port/read-size/delimiter and the backoff schedule all come from
config options.
...
from .tak_multicast import TakMulticastTransport as TakMulticastTransport
