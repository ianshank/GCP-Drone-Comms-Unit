...
from meshsa import (CompactCodec, Envelope, JsonCodec, MeshSAError, MessageKind,
                    codec_registry)
...
def _pli(ce=9999999.0):
    return Envelope(msg_id="id-1", ts=1_700_000_000.0, source_uid="user-1",
                    kind=MessageKind.PLI,
                    payload={"node": {"callsign": "FOX1", "tier": "backbone"},
...
def test_chat_roundtrip_with_and_without_to():
    c = CompactCodec()
    a = Envelope(msg_id="c1", ts=1.0, source_uid="u", kind=MessageKind.CHAT,
                 payload={"text": "rally alpha", "to": "team-1"})
    b = Envelope(msg_id="c2", ts=1.0, source_uid="u", kind=MessageKind.CHAT,
...
def test_marker_uses_position_body():
    c = CompactCodec()
    e = Envelope(msg_id="m", ts=1.0, source_uid="u", kind=MessageKind.MARKER,
                 payload={"node": {"callsign": "X"},
...
def test_status_falls_back_to_json_body():
    c = CompactCodec()
    e = Envelope(msg_id="s", ts=1.0, source_uid="u", kind=MessageKind.STATUS,
...
def test_unknown_tier_defaults_to_zero():
    c = CompactCodec()
    e = Envelope(msg_id="m", ts=1.0, source_uid="u", kind=MessageKind.PLI,
                 payload={"node": {"callsign": "X", "tier": "bogus"},
...
def test_string_too_long_raises():
    e = Envelope(msg_id="x" * 300, ts=1.0, source_uid="u", kind=MessageKind.PLI,
...
def test_incompatible_schema_rejected():
    bad = Envelope(schema_version=SCHEMA_VERSION + 9, msg_id="m", ts=1.0,
