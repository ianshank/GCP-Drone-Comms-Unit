...
def test_removed_camera_prober_keys_still_load_ignored():
    # T-5.1a removed the camera/prober groups with their consumers; configs still
    # carrying those keys must keep loading (pydantic extra="ignore" default) with
    # sibling groups unaffected.
    s = FpvSettings.from_mapping({"camera": {"fps": 60}, "prober": {"probe_margin": 5.0}})
    assert not hasattr(s, "camera")
    assert not hasattr(s, "prober")
    assert s.crsf.crsf_baud == 400000
    assert s.logger.logger_queue_len == 4096
